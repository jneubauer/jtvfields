...
class plgContentJtvfields extends JPlugin {
	...
	public function onContentBeforeSave($context, $article, $isNew) {
		...
		if(!empty($attribs->tv_video_url)){
			...
			$asset_dir = JPath::clean(JPATH_SITE.$this->params->thumbs_path);
			...
			{
				copy('http://i1.ytimg.com/vi/'.substr($url_arr['path'], 1).'/mqdefault.jpg', $asset_dir.'y'.substr($url_arr['path'], 1).'.jpg');
				$attribs->tv_video_id = 'y'.$v_id;
			}
			if ($url_arr['host'] == 'www.youtube.com')
			{
				parse_str(parse_url($attribs->tv_video_url, PHP_URL_QUERY ), $yt_arr);
				copy('http://i1.ytimg.com/vi/'.$yt_arr['v'].'/mqdefault.jpg', $asset_dir.'y'.$yt_arr['v'].'.jpg');
